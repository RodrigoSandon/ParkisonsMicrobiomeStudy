...
def mungeDataCall (csv_file):
    ...
    newFilePath = txtFilePath.replace("formatted119Bacs_addedP","munge119Bacs_output").replace(".txt","")
...
def LDscore_regression (munged_bac_output):
    ...
    LDSR_outName = "/Volumes/Passport/ldsc119Bacs_output/%s_ldscResults"%(munged_bac_output.split("/")[4].split(".")[3]) #<--only bac name
    
    #os.chdir("/Users/rodrigosandon/ldsc")
    
    cmd = "./ldsc.py \
        --rg %s,/Volumes/T7Touch/NIHSummer2021/Data/LDSR_analysis/clinicalOnlyPd.sumstats.gz \
